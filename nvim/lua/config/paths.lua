...
--- Make curl usable behind a TLS-inspecting corporate proxy.
---
--- nvim-treesitter downloads parser tarballs with curl and hardcodes its
--- arguments, so there is no way to pass a flag in. Windows curl is built
--- against Schannel, which aborts a connection when it cannot obtain the
--- revocation list for the certificate it is offered:
---
---     curl: (35) schannel: next InitializeSecurityContext failed:
---     CRYPT_E_NO_REVOCATION_CHECK - The revocation function was unable to
---     check revocation for the certificate.
---
--- A MITM proxy's generated certificate normally names no reachable CRL or
--- OCSP responder, so on such a network every parser download fails while
--- `git clone` succeeds -- git ships its own TLS stack and never consults
--- Schannel.
---
--- curl does read a config file, and CURL_HOME redirects where it looks for
--- one. So write a _curlrc carrying `ssl-no-revoke` and point CURL_HOME at it.
--- Scoped to this Neovim session and its children: the user's own curl, used
--- outside Neovim, is untouched.
---
--- Be clear about what this relaxes. The certificate chain is still verified
--- against the Windows trust store; only the revocation *check* is skipped.
--- That is curl's own documented remedy for this error, and on a network whose
--- proxy publishes no CRL there is nothing to check against in the first place.
---
--- An existing user _curlrc is carried across rather than shadowed, so settings
--- already configured there -- a proxy definition, most likely -- keep working.
---@return string|nil curl_home
function M.ensure_curl_config()
    if not is_windows then return nil end

    -- An explicit choice in the environment outranks anything done here.
    if vim.env.CURL_HOME and vim.env.CURL_HOME ~= "" then return vim.env.CURL_HOME end

    local dir = vim.fs.normalize(vim.fn.stdpath("data")) .. "/nvim-portable/curl"
    local path = dir .. "/_curlrc"

    local lines = {
        "# Generated by lua/config/paths.lua. Edits are overwritten.",
        "# Schannel cannot fetch a revocation list for a TLS-inspecting proxy's",
        "# certificate, and refuses the connection rather than proceeding. The",
        "# certificate chain is still verified; only the revocation check is",
        "# skipped.",
        "ssl-no-revoke",
    }

    -- Preserve whatever the user already had. curl would have read that file
    -- had CURL_HOME not been redirected, so ignoring it would silently drop
    -- settings.
    local appdata = vim.env.APPDATA
    if appdata and appdata ~= "" then
        local existing = io.open(vim.fs.normalize(appdata) .. "/_curlrc", "r")
        if existing then
            table.insert(lines, "")
            table.insert(lines, "# Carried over from the user _curlrc in APPDATA")
    ...
    -- Plain LF. curl parses its config line-wise and does not require CRLF.
    local content = table.concat(lines, "\n") .. "\n"

    -- Rewrite only when the content would change, so a normal startup does not
    -- touch a file that real-time antivirus then wants to rescan.
    local current = io.open(path, "rb")
    if current then
        local have = current:read("a")
        current:close()
        if have == content then
            vim.env.CURL_HOME = dir
            return dir
        end
    end
